...
@st.cache_data
def load_data():
    # In production, replace this with your actual CSV loading: pd.read_csv("Traffic Historic Data - Sheet1.csv")
    # Generating dummy historical data for testing if file isn't uploaded yet
    try:
...
def compute_holts_forecast(df, column, alpha, beta, forecast_years=[2026, 2027, 2028]):
    ...
    # Initialization
    # Level initial value is the first observation
    level = np.zeros(n)
    trend = np.zeros(n)
    
    level[0] = y[0]
    # Trend initial value is estimated as the difference between the first two data points
    if n > 1:
        trend[0] = y[1] - y[0]
    else:
        trend[0] = 0
        
    # Fit historical data points sequentially
    for t in range(1, n):
        level[t] = alpha * y[t] + (1 - alpha) * (level[t-1] + trend[t-1])
        trend[t] = beta * (level[t] - level[t-1]) + (1 - beta) * trend[t-1]
        
    # Generate the requested future period forecasts (m years ahead from the final year)
    last_level = level[-1]
    last_trend = trend[-1]
    last_year = years[-1]
    ...
    for m, f_year in enumerate(forecast_years, start=1):
        # If your data setup already reached 2025, m=1 is 2026, m=2 is 2027, etc.
        predicted_value = last_level + (m * last_trend)
        future_records.append({"YEAR": f_year, f"{column}": np.nan, "Type": "Forecasted", "Value": predicted_value})
        
    # Create clean historical tracking dataframes
    hist_records = []
    for t in range(n):
        # Fitted history value for plotting
        fit_val = level[t-1] + trend[t-1] if t > 0 else level[0]
        hist_records.append({"YEAR": years[t], f"{column}": y[t], "Type": "Historical", "Value": fit_val})
        
    return pd.DataFrame(hist_records), pd.DataFrame(future_records)

# Execute the formula engine
...
all_years = sorted(list(df_fit['YEAR'].unique()) + list(df_fore['YEAR'].unique()))
...
with tab2:
    # Build clean holistic overview log
    master_log = []
    for idx, row in df_fit.iterrows():
        master_log.append([int(row['YEAR']), f"{row[road]:,.0f}" if not np.isnan(row[road]) else "-", f"{row['Value']:,.2f}", "Historical Baseline"])
